...
function checkQuickBooks(): { connected: boolean; realmId?: string } {
  const tokenFile = path.join(os.homedir(), 'evrnew-marketing/scripts/quickbooks/qb-tokens.json')
  if (!fs.existsSync(tokenFile)) return { connected: false }
  try {
...
async function pingSearchConsoleDNS(): Promise<{ ok: boolean; token?: string }> {
  try {
    const records = await dns.resolveTxt('evrnew.com')
    const flat = records.map(r => r.join('')).join('\n')
    const match = flat.match(/google-site-verification=(\S+)/)
    return match ? { ok: true, token: match[1] } : { ok: false }
  } catch { return { ok: false } }
}
...
async function pingGoogleAds1929(): Promise<{ ok: boolean; note?: string }> {
  ...
  try {
    // Use analytics token (has adwords + analytics scopes) as primary; fall back to ads token
    const token = await getGoogleToken(true) ?? await getGoogleToken(false)
    if (!token) return { ok: false, note: 'OAuth token exchange failed' }
    const r = await fetch(
      'https://googleads.googleapis.com/v20/customers/1929533393/googleAds:search',
      {
        method: 'POST',
        headers: {
...
const ENV_INTEGRATIONS = [
  { name: 'Google Gemini',         purpose: 'Image generation only — gemini-3.1-flash-image-preview (not text/reasoning)', envKey: 'GEMINI_API_KEY', category: 'AI' },
  { name: 'xAI Grok',              purpose: 'Primary tier — grok-3 pinned. Chat, copy, ads, monitoring, Telegram', envKey: 'XAI_API_KEY',    category: 'AI' },
...
export async function GET() {
  ...
  const [ga4, gtm, brave, facebook, gads1929, scDns] = await Promise.all([
  ...
  const integrations = [
    ...
    },
    {
      ...
      category: 'Ads',
      status: gads1929.ok ? 'active' : 'pending',
      note: gads1929.ok ? undefined : 'Spencer to link account 1929533393 to manager account',
    },
    {
      name: 'Search Console (evrnew.com)',
      purpose: 'Root domain indexing & search analytics',
      category: 'SEO',
      status: scDns.ok ? 'active' : 'pending',
      note: scDns.ok ? `DNS TXT verified — click Verify in Search Console UI to activate` : 'DNS TXT record missing from evrnew.com',
    },
    {
      name: 'QuickBooks Online',
      purpose: 'Accounting — AR, AP, invoices, P&L. qb_tool.py wired into agent stack.',
      category: 'Finance',
      status: qb.connected ? 'active' : 'pending',
      note: qb.connected ? `Realm ${qb.realmId} connected` : 'Run: node ~/evrnew-marketing/scripts/quickbooks/qb-auth.js connect',
    },
  ]
