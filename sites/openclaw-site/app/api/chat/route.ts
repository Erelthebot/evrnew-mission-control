...
async function chatCluster(messages: object[]): Promise<string> {
  // MLX Metal cluster via mlx.launch jaccl (RDMA). No exo. No API key.
  const baseUrl = process.env.CLUSTER_URL || process.env.EXO_URL || 'http://127.0.0.1:52415/v1'
  const model = process.env.CLUSTER_MODEL || process.env.EXO_MODEL || 'mlx-community/Llama-3.3-70B-Instruct-4bit'
...
export async function POST(req: NextRequest) {
  ...
  // Grok-4.3 primary, local MLX JACCL cluster fallback at :52415.
  try {
    const reply = await chatXAI(messages)
    return NextResponse.json({ reply })
  } catch {
    try {
      const reply = await chatCluster(messages)
      return NextResponse.json({ reply })
    } catch (err) {
