...
const MLX_CACHE_FILE = '/tmp/mlx-cluster-state.json'

interface NodeMemory { id: string; name: string; totalGB: number; usedGB: number; freeGB: number; pct: number; gpuPct: number; pcpuPct: number; ecpuPct: number; tempC: number; powerW: number }
interface ClusterMemory { totalGB: number; usedGB: number; freeGB: number; pct: number; nodeCount: number; nodes: NodeMemory[] }

function getClusterMemory(): ClusterMemory | null {
  try {
    const raw = readFileSync(MLX_CACHE_FILE, 'utf8')
    const d = JSON.parse(raw)
    if (!d.totalGB || !d.nodeCount) return null
    return d
  } catch {
    return null
  }
...
export async function GET() {
  ...
  const cluster = getClusterMemory()
  ...
  return Response.json({
    ...
    cpu: { model: cpus[0]?.model?.replace(/\(.*\)/g, '').trim() || 'M5 Pro', count: cpus.length },
    gpu,
    ane,
