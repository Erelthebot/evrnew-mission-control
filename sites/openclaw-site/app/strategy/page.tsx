...
export default function StrategyPage() {
  ...
  const isError = /^\[Consensus unavailable|Error code:\s*\d{3}|'status':\s*'UNAVAILABLE'/.test(content)
  const sections = isError ? [] : parseSections(content)
    ...
    <div className="px-5 py-6 max-w-4xl mx-auto space-y-6">
      ...
      {isError && (
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-5">
          <p className="text-xs font-semibold text-amber-700 mb-1">Last run failed — consensus engine unavailable</p>
          <p className="text-[11px] text-amber-600">Consensus panel unreachable (Holo3 :8080, grok-3, or Llama-70B at :52415). The agent will retry on its next scheduled run. No action required.</p>
        </div>
      )}
